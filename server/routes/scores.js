...
router.get('/', (req, res) => {
  ...
  const total = db.query('SELECT COUNT(*) AS count FROM score_entries').get().count
  const entryRows = db.query('SELECT id, title, created_at FROM score_entries ORDER BY id DESC LIMIT ? OFFSET ?').all(limit, offset)
  ...
  const entries = entryRows.map((e) => ({ ...e, points: pointsByEntry.get(e.id) }))
